...
namespace MovieApp.Core.BL.Service.Impl
{
    public class StubInMemoryStorageService : IStorageService
    {
        ...
        {
            Comments comment = CommentGetById(commentId);
        ...
        {
            Comments commentToUpdate = CommentGetById(commentId);
        ...
        private void InitStubComment(int stubSize)
        {
            ...
            {
                Comments c = new(GetNextCommentId(),i+1,i+1, $"L'utente con id {i+1} ha commentato il film con id {i+1} positivamente");
                _comment.Add(c);   
            }
        }
    }
